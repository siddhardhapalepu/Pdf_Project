...
# Takes the page number string as parameter and returns the page number in INT format
def splchrtr(chrtr):
    pgnum_chrt = chrtr
...
#This funtion is to find the next page number after the questioned costs page to define a limit as in till where to search the 2018-001 pattern
...
# Extracting the page number 1 of the PDF to link it to navigate to the questioned costs page.
for i in range(15):
    pageObj_temp = pdfReader.getPage(i)
    pageContent_temp = pageObj_temp.extractText()
    pageContent_list_temp = pageContent_temp.split()
    list_length = len(pageContent_list_temp)
    if "Independent" in pageContent_list_temp:
        temp_pos = pageContent_list_temp.index("Independent")
        print(i)
        print(temp_pos)
        print(pageContent_list_temp[temp_pos + 2])
        if pageContent_list_temp[temp_pos + 2] == "Report":
            print("inside report if loop")
            pos = pageContent_list_temp.index("Report")
        else:
            continue
        dot_str = re.findall(".", pageContent_list_temp[pos + 1])
        if dot_str[0] == '.':
            print("This is dotted string")
            dot_pos = pos + 2
            while pageContent_list_temp[dot_pos].isalnum() == False:
            ...
            flag_pos = 1
            page = splchrtr(pageContent_list_temp[pos + 1])
            print("Page number of first page is:", page)
            print("current page is (else):", i)
        break
